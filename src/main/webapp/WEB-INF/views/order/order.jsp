	...
	<div class="login_area">
		...
		<c:if test="${member != null}">
			<div class="login_success_area">
				...
				<span>충전금액 : <fmt:formatNumber value="${member.memberMoney}" pattern="\#,###.##" /></span>
...
<div class="content_area">
	...
	<div class="content_main">
		<!-- 회원 정보 -->
		<div class="member_page_div">
			<table class="table_text_align_center memberPage_table">
				<tbody>
					<tr>
						<th style="width: 25%;">주문자</th>
						<td style="width: *">${memberPage.memberName} | ${memberPage.memberEmail}</td>
					</tr>
				</tbody>
			</table>
		</div>
					...
					<table>
						...
						<tbody>
							<tr>
								<th>이름</th>
								<td>
									<input class="orderAddress_input">
								</td>
							</tr>
							<tr>
								<th>주소</th>
			...
			<table class="goods_table">
				...
				<tbody>
					<c:forEach items="${orderList}" var="ol">
						<tr>
							<td>
								<div class="image_wrap" data-bookid="${ol.imageList[0].productNum}" data-path="${ol.imageList[0].uploadPath}" data-uuid="${ol.imageList[0].uuid}" data-filename="${ol.imageList[0].fileName}">
									<img>
								</div>
							</td>
							...
							<td class="goods_table_price_td">
								...
								<input type="hidden" class="individual_totalPrice_input" value="${ol.salePrice * ol.orderCount}">													
			...
			<div class="total_info_price_div">
				<ul>
					...
					<li>
						...
						<span class="delivery_price_sapn">100000</span>원
					</li>			
					<li class="price_total_li">
						<strong class="price_sapn_label total_price_label">최종 결제 금액</strong>
	...
	<form class="order_form" action="/order" method="post">
		<!-- 주문자 회원번호 -->
		<input name="memberId" value="${memberPage.memberId}" type="hidden">
		<!-- 주소록 & 받는이-->
		<input name="addressee" type="hidden">
		<input name="memberAddress" type="hidden">
		<!-- 상품 정보 -->
	</form>
	...
	$(".order_btn").on("click", function(){
		...
		$(".addressInfo_input_div").each(function(i, obj){
			if($(obj).find(".selectAddress").val() === 'T'){
				$("input[name='orderAddress']").val($(obj).find(".addressee_input").val());
				$("input[name='memberAddress']").val($(obj).find(".address1_input").val());
			}
		});	
